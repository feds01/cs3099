...
describe('Follower endpoints testing ', () => {
    ...
    afterAll(async () => {
        const deleteFollowee = await request.delete('/user/followee').auth(followeeRes.body.token, {type: 'bearer'});
        const deleteFollower1 = await request.delete('/user/follower1').auth(follower1Res.body.token, {type: 'bearer'});
        const deleteFollower2 = await request.delete('/user/follower2').auth(follower2Res.body.token, {type: 'bearer'});
        ...
        const followDoc = await Follower.count({ $or: [ {follower: {$in: [follower1!.id, follower2!.id]}}, {followee: followee!.id} ] });
    ...
    it('follower1 follows followee', async () => {
        const followResponse = await request.post(`/user/followee/follow`).auth(follower1Res.body.token, { type: 'bearer' });
        expect(followResponse.status).toBe(201);

        const followDoc = await Follower.count({ follower: follower1!.id, following: followee!.id });
    ...
    it('follower2 follows followee', async () => {
        const followResponse = await request.post('/user/followee/follow').auth(follower2Res.body.token, { type: 'bearer' });
        expect(followResponse.status).toBe(201);
    ...
    it('follower2 unfollows followee', async () => {
        const unfollowResponse = await request.delete('/user/followee/follow').auth(follower2Res.body.token, { type: 'bearer' });
        expect(unfollowResponse.status).toBe(200);

        const followDoc = await Follower.count({ follower: follower2!.id, following: followee!.id });
        expect(followDoc).toBe(0);
    });

    it('check if follower1 is following followee and follower2 is not', async () => {
        const isfollowingResponse = await request.get('/user/followee/follow').auth(follower1Res.body.token, { type: 'bearer' });
        expect(isfollowingResponse.status).toBe(200);
        expect(isfollowingResponse.body.following).toBe(true);

        const notFollowingResponse = await request.get('/user/followee/follow').auth(follower2Res.body.token, { type: 'bearer' });
