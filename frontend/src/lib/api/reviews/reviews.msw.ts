...
export const getPostReviewIdCommentMock = () => ({
...
export const getGetReviewIdMock = () => ({
    status: faker.helpers.randomize(['ok']),
    review: faker.helpers.randomize([
        {
            publication: {
                id: faker.random.word(),
                name: faker.random.word(),
...
export const getReviewsMSW = () => [
    ...
    rest.post('*/review/:id/comment', (req, res, ctx) => {
        return res(ctx.delay(1000), ctx.status(200, 'Mocked status'), ctx.json(getPostReviewIdCommentMock()));
    }),
