...
import { PatchComment, SuccessStatus } from '.././models';

export const getGetThreadIdMock = () => ({
...
export const getCommentsMSW = () => [
    rest.get('*/thread/:id', (_req, res, ctx) => {
        return res(ctx.delay(1000), ctx.status(200, 'Mocked status'), ctx.json(getGetThreadIdMock()));
    }),
    rest.delete('*/thread/:id', (_req, res, ctx) => {
        return res(ctx.delay(1000), ctx.status(200, 'Mocked status'));
    }),
    rest.get('*/comment/:id', (_req, res, ctx) => {
        return res(ctx.delay(1000), ctx.status(200, 'Mocked status'), ctx.json(getGetCommentIdMock()));
    }),
    rest.patch('*/comment/:id', (_req, res, ctx) => {
        return res(ctx.delay(1000), ctx.status(200, 'Mocked status'), ctx.json(getPatchCommentIdMock()));
    }),
