...
router.post('/:id/follow', paramValidator, ownerAuth, async (req, res) => {
    const { id: followingId } = req.params;
    const { id: followerId } = req.token.data; // req.token is from ownerAuth

    // check if the user is trying to following himself/herself
    if (followingId === followerId) {
        return res.status(401).json({
            status: false,
            message: error.SELF_FOLLOWING,
        });
    }
    ...
    const following = await User.findById(followingId).exec();
...
router.delete('/:id/follow', paramValidator, ownerAuth, async (req, res) => {
    const { id: followingId } = req.params;
    ...
    // then exit early and don't create the new follower link.
    const follower = await User.findById(followerId).exec();
    const followee = await User.findById(followingId).exec();
    ...
    const link = await Follower.findOneAndDelete({ follower: follower._id, following: followee._id }).exec();
...
router.get('/:id/follow', paramValidator, ownerAuth, async (req, res) => {
    const { id: followingId } = req.params;
    const { id: followerId } = req.token.data;
...
router.get('/:id/followers', paramValidator, ownerAuth, async (req, res) => {
    const { id } = req.params;
...
router.get('/:id/following', paramValidator, ownerAuth, async (req, res) => {
    const { id } = req.params;
